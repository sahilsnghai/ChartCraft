...
class BoxplotChartBase(BaseChart):
    ...
    def _get_axis_config(self) -> Dict[str, Any]:
        ...
        if self.config.get("x"):
                ...
                "xAxis": {
                    ...
                    "labels": {
                        "style": {
                            ...
                            "fontSize": self.config.get("xAxis", {}).get(
                                "labelFontSize", 12
                            ),
                            "fontWeight": self.config.get("xAxis", {}).get(
                                "labelFontWeight"
                            ),
                        }
                    },
            ...
            return {
                "xAxis": {"categories": [self.config.get("outlierAttribute", "")]},
                "yAxis": AxisConfig(self.df, self.config).get_y_axis(),
            }
...
class BoxplotChart(BoxplotChartBase):
    ...
    def _build_boxplot_series(self) -> List[Dict[str, Any]]:
        ...
        if (
            ...
            and self.config.get("outlierAttribute")
        ):
    ...
    def _get_series_with_x_y_outlier(self) -> List[Dict[str, Any]]:
        ...
        for group in groups:
            ...
            outlier_attributes = outlier_group_df[
                self.config["outlierAttribute"]
            ].to_list()
    ...
    def _get_series_with_y_outlier(self) -> List[Dict[str, Any]]:
        ...
        # Build outlier data
        outlier_data = [
            [0, value, attr] for value, attr in zip(outlier_values, outlier_attributes)
        ]
...
class CustomBoxplotChart(BoxplotChartBase):
    ...
    def _get_series_with_x_y_outlier(self) -> List[Dict[str, Any]]:
        ...
        for group in groups:
            ...
            for i in range(len(outliers_measure)):
                outlier_data.append([group, outliers_measure[i], outliers_attribute[i]])
    ...
    def _get_series_with_x_y(self) -> List[Dict[str, Any]]:
        ...
        for group in groups:
            ...
            # Add to outlier data
            for i in range(len(outliers_measure)):
                outlier_data.append([0, outliers_measure[i], outliers_attribute[i]])

        return [
            {"type": "boxplot", "name": self.config["x"], "data": boxplot_data},
            {
                "type": "scatter",
                "name": "Outliers",
