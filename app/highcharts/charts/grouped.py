...
class GroupedChart(BaseChart):
    ...
    def _get_chart_type(self) -> str:
        return self.config["type"].split("-")[1]  # Extract chart type from 'grouped-{type}'
        ...
        try:
            ...
            series = [{
                ...
                "data": [
                    ...
                    for data_index in enumerate(data)
                ],
            ...
            return {
                "series": [],
                "chart": {"type": self._get_chart_type()}
            }
    ...
    def _get_x_axis_with_category(self) -> tuple[Dict[str, Any], List[float]]:
        ...
        if self.config.get('x'):
            ...
            x_categories = self._build_category_structure(self.df, self.config['x'], data)
            categories = {"categories": x_categories}
        
        return categories, data
    
    def _build_category_structure(self, df: pl.DataFrame, levels: List[str], data: List[float]) -> List[Dict[str, Any]]:
