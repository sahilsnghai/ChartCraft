...
class HeatmapChart(BaseChart):
    ...
    def get_series(self) -> Dict[str, Any]:
        ...
        elif self.config.get("value") and (
            self.config.get("x") or self.config.get("y")
        ):
    ...
    def _get_series_with_x_y_value(self) -> Dict[str, Any]:
        ...
        result = {
            ...
            "yAxis": {"title": {"text": self.config["y"]}, "categories": y_categories},
        }
    ...
    def _get_series_with_value_and_axis(self) -> Dict[str, Any]:
        ...
        if self.config.get("y"):
            ...
            matrix = self.df[self.config["value"][0]].to_list()
            matrix_data = [[0, row, matrix[row]] for row in range(len(y_categories))]
            ...
            matrix = self.df[self.config["value"][0]].to_list()
            matrix_data = [[row, 0, matrix[row]] for row in range(len(matrix))]

            result = {"yAxis": {"categories": [self.config["value"][0]]}}
...
class TreemapChart(BaseChart):
    ...
    def _get_series_with_x_y_value(self) -> Dict[str, Any]:
        ...
        for row, parent_name in enumerate(parent_names):
            ...
            # Add parent node
            matrix_data.append(
                {"name": parent_name, "id": str(row), "color": self._get_color(row)}
            )
        ...
        result = {
            ...
            "chart": {"type": self.chart_type},
            "xAxis": AxisConfig(self.df, self.config).get_x_axis(),
            "yAxis": {"title": {"text": self.config["y"]}, "categories": parent_names},
        }
...
class WaterfallChart(BaseChart):
    ...
    def get_series(self) -> Dict[str, Any]:
        ...
        if self.config.get("x"):
            self.df, x_values = DataProcessor.replace_with_index(
        ...
        if (
            self.config.get("x")
            and self.config.get("y")
            and self.config.get("difference")
        ):
            return self._get_series_with_difference()
        elif (
            self.config.get("x")
            and self.config.get("y")
            and self.config.get("changePercent")
        ):
    ...
    def _get_series_basic(self) -> Dict[str, Any]:
        ...
        if self.config.get("x"):
            ...
            self.df = self.df.with_columns(
                pl.Series(values=x_values).alias("categories")
            )
            names = self.df["categories"].to_list()
        else:
            names = [""]
        ...
        result = {
            "series": [{"type": "waterfall", "name": self.config["y"], "data": data}],
            "chart": {"type": self.chart_type},
        }
...
class BellCurveChart(BaseChart):
    ...
    def get_series(self) -> Dict[str, Any]:
        ...
        result = {
            ...
            "chart": {"type": self.chart_type},
            "xAxis": [
                {"title": {"text": self.config["y"]}, "alignTicks": False},
                {
            ...
            "yAxis": [
                {"title": {"text": self.config["y"]}},
                {"title": {"text": "Probability Density"}, "opposite": True},
            ],
        }
...
class StreamgraphChart(BaseChart):
    ...
    def get_series(self) -> Dict[str, Any]:
        ...
        for dimension in dimensions:
            ...
            series = {"type": "streamgraph", "name": dimension, "data": data}
    ...
    def get_series(self) -> Dict[str, Any]:
        ...
        overall_trend = next(
            (
                ...
                if any(
                    sub in col.lower()
                    and (col.lower().startswith(sub) or col.lower().endswith(sub))
        ...
        result = {
            ...
            "chart": {"type": "column"},
            "xAxis": AxisConfig(self.df, self.config).get_x_axis(),
            "yAxis": [{}, {"opposite": True}],  # Left y-axis  # Right y-axis for change
        }
