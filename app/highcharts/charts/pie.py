...
class PieChartBase(BaseChart):
    ...
    def _build_pie_series(self) -> List[Dict[str, Any]]:
        ...
        names = self.x_col_distinct_values if self.config.get('x') else self.df[self.config['y']].to_list()
...
class SunburstChart(BaseChart):
    ...
    def get_series(self) -> Dict[str, Any]:
        ...
        parent_values = self.df.group_by(self.config['x']).sum().sort(by=self.config['x'])[self.config['y']].to_list()
