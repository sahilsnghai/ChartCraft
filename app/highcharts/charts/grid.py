...
class GridChart(BaseChart):
    ...
    def _build_columns_config(self, grid_config: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for idx, (col_name, dtype) in enumerate(self.df.schema.items()):
            if dtype in [pl.Float64, pl.Int64]:
                # Measure column
                column_config = self._get_measure_column_config(col_name, idx, grid_config)
                # Add currency formatting if unit is specified
                if self.config.get("unit"):
                    column_config = self._add_currency_formatting(column_config, self.config["unit"])
                columns.append(column_config)
                
            elif dtype in [pl.Utf8, pl.Boolean]:
                # Dimension column
                columns.append(self._get_dimension_column_config(col_name, idx, grid_config))
        
        return columns
    
    def _get_measure_column_config(self, col_name: str, idx: int, grid_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _get_dimension_column_config(self, col_name: str, idx: int, grid_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _add_currency_formatting(self, column_config: Dict[str, Any], unit: str) -> Dict[str, Any]:
        """Add currency formatting to column configuration."""
        currency_map = {
            "€": "EUR", "kr": "SEK", "฿": "THB", "#": None,
            "Mex$": "MXN", "R$": "BRL", "A$": "AUD", "C$": "CAD",
            "$": "USD", "JP¥": "JPY", "₩": "KRW", "NT$": "TWD",
            "¥": "CNY", "₹": "INR", "£": "GBP", "zł": "PLN"
        }
