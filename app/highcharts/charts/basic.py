...
class BasicCartesianChart(BaseChart):
    ...
    def _get_series_with_color_column(self) -> List[Dict[str, Any]]:
        ...
        for group_name, group_df in grouped:
            ...
            data = [
                ...
                for row in group_df.select([self.config['x'], self.config['y']]).iter_rows()
            ]
    ...
    def _get_measures_config(self) -> Dict[str, Any]:
        ...
        categories = self.config['y'] if isinstance(self.config['y'], list) else [self.config['y']]
        ...
        return {
            ...
            'yAxis': {
                'title': {
                    'text': ', '.join(categories[:-1]) + f' and {categories[-1]}'
                }
            }
        }
