...
class TooltipBuilder:
    ...
    def create_tooltip(config: ChartConfig, chart_type: str) -> Dict[str, Any]:
        ...
        return TooltipBuilder._default_tooltip(
            config, base_tooltip, value_prefix, value_suffix
        )

    @staticmethod
    def _default_tooltip(
        config: ChartConfig,
        base_tooltip: Dict[str, Any],
        value_prefix: str,
        value_suffix: str,
    ) -> Dict[str, Any]:
        """Create default tooltip with value formatting."""
        point_format = "{point.y:,.2f}"
        point_format = TooltipBuilder._format_point_format(
            value_prefix, value_suffix, point_format, "{point.series.name}: "
        )

        return {**base_tooltip, "pointFormat": point_format}

    @staticmethod
    def _heatmap_tooltip(
        config: ChartConfig, base_tooltip: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _pie_tooltip(
        ...
        return {**base_tooltip, "headerFormat": "", "pointFormat": point_format}

    @staticmethod
    def _sunburst_tooltip(
        config: ChartConfig, base_tooltip: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        point_format = "{point.value:,.2f}"
        point_format = TooltipBuilder._format_point_format(
            value_prefix, value_suffix, point_format, "{point.name}: "
        )

        return {**base_tooltip, "headerFormat": "", "pointFormat": point_format}

    @staticmethod
    def _scatter_tooltip(
        config: ChartConfig, base_tooltip: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        point_format_1 = TooltipBuilder._format_point_format_scatter(
            value_prefix1, value_suffix1, point_format1
        )
        point_format_2 = TooltipBuilder._format_point_format_scatter(
            value_prefix2, value_suffix2, point_format2
        )
        ...
        if isinstance(config.get("dimension"), list):
            ...
            point_format = (
                "<b>{series.userOptions.color_name}</b>: {point._name}</br>"
                + point_format
            )
    ...
    def _bubble_tooltip(
        ...
        if (
            config.get("color")
            and isinstance(config.get("dimension"), str)
            and config.get("radius")
        ):
    ...
    def _split_bubble_tooltip(
        ...
        point_format = "{point.y:,.2f}"
        point_format = TooltipBuilder._format_point_format(
            value_prefix, value_suffix, point_format, "<b>{series.userOptions.y}</b>: "
        )
    ...
    def _waterfall_tooltip(
        ...
        if (
            config.get("value")
            or config.get("difference")
            or config.get("changePercent")
        ):
            point_format = "{point.value:,.2f}"
            point_format = TooltipBuilder._format_point_format(
                value_prefix,
                value_suffix,
                point_format,
                "<b>{series.userOptions.attribute}</b>: ",
            )

            if config.get("difference"):
                return {
                    **base_tooltip,
                    "pointFormat": "<b>{series.name}</b>: {point.y}<br>" + point_format,
                }
            else:
                return {
                    **base_tooltip,
    ...
    def _boxplot_tooltip(
        ...
        point_format = TooltipBuilder._format_point_format(
            ...
            value_suffix,
            point_format,
            "<b>{series.userOptions.measure}</b>: ",
        )

        return {
            **base_tooltip,
            "headerFormat": "",
            "pointFormat": "<b>{series.userOptions.attribute}</b>: {point.outlier_name}</br>"
            + point_format,
        }

    @staticmethod
    def _treemap_tooltip(
        config: ChartConfig, base_tooltip: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _bell_curve_tooltip(
        ...
        point_format = TooltipBuilder._format_point_format(
            value_prefix, value_suffix, "{point.y}", point_format
        )

        return {**base_tooltip, "headerFormat": "", "pointFormat": point_format}

    @staticmethod
    def _area_range_tooltip(
        config: ChartConfig, base_tooltip: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    @staticmethod
    def _format_point_format(
        value_prefix: str, value_suffix: str, point_format: str, point_series_name: str
    ) -> str:
        ...
        if has_prefix and has_suffix:
            return (
                f"{point_series_name}{value_prefix}{point_format} {value_suffix}</br>"
            )
        elif has_prefix:
    ...
    @staticmethod
    def _format_point_format_scatter(
        value_prefix: str, value_suffix: str, point_format: str
    ) -> str:
...
class PlotOptionsBuilder:
    ...
    def _boxplot_plot_options(config: ChartConfig) -> Dict[str, Any]:
        ...
        return {"boxplot": {"dataLabels": data_labels}}
    ...
    def _area_range_plot_options(config: ChartConfig) -> Dict[str, Any]:
        ...
        return {"area": {"stacking": None, "dataLabels": data_labels}}
    ...
    def _bar_stack_plot_options(config: ChartConfig) -> Dict[str, Any]:
        ...
        return {key: {"stacking": "normal", "dataLabels": data_labels}}
    ...
    def _bar_stack_percent_plot_options(config: ChartConfig) -> Dict[str, Any]:
        ...
        return {key: {"stacking": "percent", "dataLabels": data_labels}}
    ...
    def _streamgraph_plot_options(config: ChartConfig) -> Dict[str, Any]:
        ...
        return {"series": {"dataLabels": data_labels}}
    ...
    def _pie_plot_options(config: ChartConfig) -> Dict[str, Any]:
        ...
        return {
            "pie": {"stacking": None, "showInLegend": True, "dataLabels": data_labels}
        }
    ...
    def _sunburst_plot_options(config: ChartConfig) -> Dict[str, Any]:
        ...
        return {"sunburst": {"stacking": None, "dataLabels": data_labels}}
    ...
    def _scatter_plot_options(config: ChartConfig) -> Dict[str, Any]:
        ...
        return {chart_type: {"stacking": None, "dataLabels": data_labels}}
            ...
            "packedbubble": {
                ...
                "dataLabels": {
                    ...
                    "style": {
                        "textOutline": "none",
                        "fontWeight": "normal",
                        "color": config.get("dataLabels", {})
                        .get("style", {})
                        .get("color", "#000000"),
                        "fontSize": config.get("dataLabels", {})
                        .get("style", {})
                        .get("fontSize", 11),
                    },
                    "allowOverLap": config.get("dataLabels", {}).get(
                        "allowOverLap", False
                    ),
                },
            }
    ...
    def _step_line_plot_options(config: ChartConfig) -> Dict[str, Any]:
        ...
        return {"line": {"stacking": None, "dataLabels": data_labels}}
    ...
    def _get_data_labels_config(config: ChartConfig) -> Dict[str, Any]:
        ...
        if isinstance(data_labels_config, bool) or data_labels_config is None:
            data_labels_config = {
                "enabled": (
                    data_labels_config if data_labels_config is not None else True
                )
            }
        ...
        return {
            ...
            "style": {
                "textShadow": data_labels_config.get("style", {}).get(
                    "textShadow", False
                ),
