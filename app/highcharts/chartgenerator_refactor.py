...
class ChartGenerator:
    ...
    def generate_viz(
        ...
        try:
            ...
            for index, chart_config in enumerate(config):
                ...
                if index == 0:
                    chart_config = self._apply_advance_settings(
                        advance_settings, chart_config
                    )
                else:
                    chart_config = self._preserve_data_labels(
                        advance_settings, chart_config
                    )

                # Create chart instance using factory
                chart_instance = ChartFactory.create_chart(
                    chart_config["type"], df, ChartConfig(chart_config)
                )
                ...
                high_chart_config = self._update_chart_with_series(
                    high_chart_config, index, chart_config, chart_instance
                )

                # Initialize chart options for the first chart
                if index == 0:
                    high_chart_config = self._initialize_chart_options(
                        high_chart_config, chart_config, chart_instance
                    )
            ...
            if len(config) > 1 and not is_grid_or_map:
                high_chart = self._configure_multi_chart_y_axis(
                    config, high_chart, advance_settings
                )
    ...
    def _apply_advance_settings(
        self, advance_settings: Dict[str, Any], chart_config: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _update_chart_with_series(
        self,
        high_chart_config: Dict[str, Any],
        index: int,
        ...
        tooltip_config = self.tooltip_builder.create_tooltip(
            ChartConfig(chart_config), chart_config["type"]
        )
        high_chart_config["tooltip"] = tooltip_config

        # Add plot options
        plot_options = self.plot_options_builder.create_plot_options(
            ChartConfig(chart_config), chart_config["type"]
        )
        ...
        high_chart_config = self._update_scrollbar(
            high_chart_config, chart_config, index
        )

        return high_chart_config

    def _initialize_chart_options(
        self,
        high_chart_config: Dict[str, Any],
        chart_config: Dict[str, Any],
        chart_instance,
        ...
        high_chart_config["exporting"] = {
            "enabled": chart_config.get("exporting", False)
        }
        ...
        if chart_config.get("max"):
            if isinstance(high_chart_config["xAxis"], dict):
                high_chart_config["xAxis"].update(
                    {"scrollbar": {"enabled": True}, "max": chart_config["max"]}
                )
            else:
    ...
    def _configure_multi_chart_y_axis(
        self,
        config: List[Dict[str, Any]],
        ...
        for index, chart_config in enumerate(config):
            if chart_config.get("opposite"):
                y_axis_config.append(
                    self._build_y_axis_config(
                        advance_settings, chart_config, opposite=True
                    )
                )
                ...
                y_axis_config.append(
                    self._build_y_axis_config(
                        advance_settings, chart_config, opposite=False
                    )
                )
        ...
        if not opposite_found and y_axis_names:
            ...
            if "yAxis" in high_chart and high_chart["yAxis"]:
                ...
                high_chart["yAxis"]["title"]["text"] = advance_settings.get(
                    "yAxis", {}
                ).get("commonName", y_label_text)

        return high_chart

    def _build_y_axis_config(
        self,
        advance_settings: Dict[str, Any],
        chart_config: Dict[str, Any],
        opposite: bool = False,
...
def generate_viz(
    advance_settings: Dict[str, Any], df: pl.DataFrame, config: List[Dict[str, Any]]
) -> Dict[str, Any]:
    """Generate visualization using the new class-based architecture.

    This function maintains the same signature as the original generate_viz
    to ensure backward compatibility.
    """
    generator = ChartGenerator()
    return generator.generate_viz(advance_settings, df, config)
